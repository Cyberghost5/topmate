<?php
    ...
    */
    ...
    $allow=["settings","register","login","logins","login2","get-user-code","verify-user-code","update-user-pass","update-user-pin","save-message"];
    ...
    if(isset($_GET["settings"])):
        ...
        elseif(isset($_GET["login"])){
         ...
         elseif(isset($_GET["logins"])){
            $controller = new AccountAccess;
            echo $controller->loginUserFingerprint();
            exit();
        }
          elseif(isset($_GET["login2"])){
            $controller = new AccountAccess;
            echo $controller->loginname();
            exit();
        }
        ...
        //Generate Bellbank Account
        if (isset($_POST["generate-bellbank-account"])) {
    global $controller;

    // Ensure $controller is initialized and is an object
    if (!isset($controller) || !is_object($controller)) {
        error_log("Controller is not an object. Reinitializing.");
        $controller = new Subscriber(); // Or the correct controller class
    }

    // Call the method and handle the response
    if (is_object($controller)) {
        $msg = $controller->generateBellbankAccount();
    } else {
        error_log("Controller reinitialization failed.");
    }
}
           //Generate Strowallet Account
        if(isset($_POST["generate-strowallet-account"])){
			$account_type=$_POST["generate-strowallet-account"];
			
        $msg=$controller->generateStrowalletAccount($account_type);
        }
        //Generate Billstack Account
         if(isset($_POST["generate_billstack_psb"])){
             $bank="9PSB";
         $msg = $controller->createBillstackVirtualAccounts($bank); 
        }
        
        
          //Generate Billstack Account
       // if(isset($_POST["generate_billstack_providus"])){
          //  $bank="PROVIDUS";
        // $msg = $controller->createBillstackVirtualAccounts($bank); 
       // }
        
            //Generate Billstack Account
        if(isset($_POST["generate_billstack_safehaven"])){
         $bank="SAFEHAVEN";
         $msg = $controller->createBillstackVirtualAccounts($bank); 
        }
        
         if(isset($_POST["generate_billstack_palmpay"])){
         $bank="PALMPAY";
         $msg = $controller->createBillstackVirtualAccounts($bank); 
        }
              //Generate Billstack Account
        if(isset($_POST["generate_billstack_bankly"])){
         $bank="BANKLY";
         $msg = $controller->createBillstackVirtualAccounts($bank); 
        }
        ...
        function getDataIfAny($page){
            ...
            switch($page){
                ...
                case "homepage":
                    ...
                    $data[3]=$controller->displayFlashNotification();
                    $data[4]=$controller->getBillstackAccounts();
                ...
                case "fund-wallet":
                    ...
                    $data[1]=$controller->getApiConfiguration();
                    $data[2]=$controller->getSiteSettings();
                     $data[3]=$controller->getBillstackAccounts();
                     $data[4]=$controller->getStrowalletAccounts();
                    $controller->recordLastActivity();
                    return $data;
                    break;
