<?php
	...
	class Subscriber extends Controller{
		...
		public function getProfileInfo(){
        	...
        	//Generate UnitedApi Account
		
		public function generateBellbankAccount(){
		    $this->model->generateBellbankAccount($this->userId);
		}
		...
		public function purchaseAirtime(){
			...
			$check=$this->model->verifyTransactionPin($this->userId,$transkey);
			$ported_number = "false";
			$transref = "AIRTIME".rand(100,999)."_".$transref;
			...
			if($curl_error){
				return $this->createPopMessage("Error!!", "Connection error: " . $curl_error, "red");
			}
			
			if(!$result || !is_object($result)){
				return $this->createPopMessage("Error!!", "Invalid response from API. Please try again later.", "red");
			}
			...
			if(isset($result->status) && ($result->status == "success" || $result->status == "processing")){
				...
				$error_msg = isset($result->msg) ? $result->msg : "Unknown error occurred";
				return $this->createPopMessage("Error!!", $error_msg, "red");
			}
		}
		else{
			return $this->createPopMessage("Error!!","Incorrect Pin, Please Try Again.","red");
		...
		public function purchaseAlphaTopup(){
			...
			$check=$this->model->verifyTransactionPin($this->userId,$transkey);
			$ported_number = "false";
			$transref = "Alpha".rand(100,999)."_".$transref;
			if(is_object($check)){
		...
		public function purchaseData(){
			...
			$check=$this->model->verifyTransactionPin($this->userId,$transkey);
			$ported_number = "false";
			$transref = "DATA".rand(100,999)."_".$transref;
			if(isset($_POST["ported_number"])){
			    if($_POST["ported_number"] == "on"){$ported_number = "true";}
			}
		...
		public function purchaseDataPin(){
			...
			$check=$this->model->verifyTransactionPin($this->userId,$transkey);
			$ported_number = "false";
			$transref = "DATAPIN".rand(100,999)."_".$transref;
			if(isset($_POST["ported_number"])){
			    if($_POST["ported_number"] == "on"){$ported_number = "true";}
			}
		...
		public function purchaseExamPinToken(){
			...
			$transkey=strip_tags($transkey);
			$check=$this->model->verifyTransactionPin($this->userId,$transkey);
			$transref = "EXAMPIN".rand(100,999)."_".$transref;
			if(is_object($check)){
		...
		public function purchaseElectricityToken(){
			...
			$transkey=strip_tags($transkey);
			$check=$this->model->verifyTransactionPin($this->userId,$transkey);
			$transref = "BILL".rand(100,999)."_".$transref;
			if(is_object($check)){
   ...
   //Get Total Spent In A Month
     function monthlySpent(){
		...
		public function performFundsTransfer(){
			...
			//----------------------------------------------------------------------------------------------------------------
		// STROWALLET ACCOUNT MANAGEMENT
		//----------------------------------------------------------------------------------------------------------------
		

		//Generate Strowallet Account
		
		public function generateStrowalletAccount($account_type){
		 file_put_contents('logstro_file.txt',json_encode($account_type));
        $data=	$this->model->generateStrowalletAccount($this->userId,$account_type);
        //var_dump($data);die;
        	if($data["account_generated"] == true){
					return $this->createPopMessage("Success!!","Account generated Successfully","green");
				
				}
				else{return $this->createPopMessage("Error!!",$data["msg"],"red");}
		...
		//----------------------------------------------------------------------------------------------------------------
		// GET BILLSTACK ACCOUNCT
		//----------------------------------------------------------------------------------------------------------------
		
		//Generate Kuda Account
		
		public function getBillstackAccounts(){
		   $data= $this->model->getBillstackAccounts($this->userId);
		   return $data;
		}

//----------------------------------------------------------------------------------------------------------------
		// GET Strowallet ACCOUNCT
		//----------------------------------------------------------------------------------------------------------------
	
		public function getStrowalletAccounts(){
		   $data= $this->model->getStrowalletAccounts($this->userId);
		   return $data;
		}
		//----------------------------------------------------------------------------------------------------------------
		...
		public function displayFlashNotification(){
			$data=$this->model->getFlashNotification();
			return $data;
		}
		...
		//----------------------------------------------------------------------------------------------------------------
		// PAGA ACCOUNCT MANAGEMENT By // PAGA ACCOUNT MANAGEMENT By Daltech Digital Resources 07035890194
		//----------------------------------------------------------------------------------------------------------------
	...
	// Payvessel ACCOUNCT MANAGEMENT
		...
		//----------------------------------------------------
		//CREATE BILLSTACK ACCOUNT
		//-------------------------------------------------------

 public function createBillstackVirtualAccounts($bank){
          $userinfo = $this->getProfileInfo();
          //$details=$this->model->getApiConfiguration();
			
          $model = new ApiModel;
		  $api_details=$model->getApiDetails();
          $ApiAccess = new ApiAccess;

          $billstack_public_key=$ApiAccess->getConfigValue($api_details,"billstackPublicKey");
          $billstack_secret_key=$ApiAccess->getConfigValue($api_details,"billstackSecretKey");
          $billstack_charges=$ApiAccess->getConfigValue($api_details,"billstackCharges");
          $billstack_host = "https://api.billstack.co/v2/thirdparty/generateVirtualAccount/";
			$ref = str_shuffle(substr("ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjklmnpqrstuvwxyz1234567890",0,12));			 

				$curl = curl_init();
                
                curl_setopt_array($curl, [
                  CURLOPT_URL => $billstack_host,
                  CURLOPT_RETURNTRANSFER => true,
                  CURLOPT_ENCODING => "",
                  CURLOPT_MAXREDIRS => 10,
                  CURLOPT_TIMEOUT => 30,
                  ...
                  CURLOPT_POSTFIELDS => json_encode([
                    ...
                    'firstName' => $userinfo->sFname,
                    'phone' => $userinfo->sPhone,
                    'reference' => $ref,
                    'lastName' => $userinfo->sLname,
                    'bank' => $bank
                  ]),
                  CURLOPT_HTTPHEADER => [
		...
		if(is_array($res["data"]['account'])){
			foreach( $res["data"]['account'] as $value){
				$accountNumber=$value['account_number'];
			}
			
	
		
			$check=$this->model->saveBillstackVirtualAccount($this->userId,$accountNumber,$ref,$bank);
			
			
			if($check == 0){
				$message="Virtual account has been generated for you";
				
				return $this->createPopMessage("Success!!","$message","green");
			    ...
			    $error=$res['message'];
			return $this->createPopMessage("Error!!","Server error: $error","red");	
			}
