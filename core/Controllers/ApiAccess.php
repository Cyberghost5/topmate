...
public function verifyUnitedapiRef($account_number, $token, $payload) {
    return $result = $this->model->verifyUnitedapiRef($account_number, $token, $payload);
}

// Record Transaction & Debit User for Billstack
public function recordUnitedapiTransaction($userid, $servicename, $servicedesc, $amount, $userbalance, $ref, $status) {
    $response = array();
    $oldbalance = (float) $userbalance;
    $newbalance = $oldbalance + $amount;

    $result = $this->model->recordUnitedapiTransaction($userid, $servicename, $servicedesc, $ref, $amount, $oldbalance, $newbalance, $status);
    $response["status"] = "success";
    ...
    return $response;
        ...
        public function verifyNetworkrId($network){
            $result = $this->model->verifyNetworkrId($network);
            $response = array();
            if(is_object($result)){
                $response = (array) $result;
                $response["status"] = "success";
        ...
        public function updateTransactionWithRealResponse($ref,$response){
        	...
        	//Record Transaction & Debit User
        public function recordAspfiyTransaction($userid,$servicename,$servicedesc,$amount,$userbalance,$ref,$status){
            $response = array();
            $oldbalance = (float) $userbalance;
            $newbalance = $oldbalance + $amount;

            $result=$this->model->recordAspfiyTransaction($userid,$servicename,$servicedesc,$ref,$amount,$oldbalance,$newbalance,$status);
            $response["status"] = "success";
            $response["amount"] = $amount;
            $response["oldbal"] = $oldbalance;
            $response["newbal"] = $newbalance;
        ...
        public function verifyBillstackNotification($key,$email){
		 ...
		 public function recordBillstackTransaction($userid,$servicename,$servicedesc,$amount,$userbalance,$ref,$status){
            $response = array();
            $oldbalance = (float) $userbalance;
            $newbalance = $oldbalance + $amount;
      ...
      //Verify Paystack Transaction
		public function verifyStrowalletTransaction($accountNumber,$sessionId,$amount_paid){
			$result=$this->model->verifyStrowalletTransaction($accountNumber,$sessionId,$amount_paid);
            $response = array();
			if(is_object($result)){
                $response["status"] = "success";
                $response["userid"] = $result->userid;
                $response["balance"] = $result->userbalance;
