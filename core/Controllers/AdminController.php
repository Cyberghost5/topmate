<?php
	...
	class AdminController extends Controller{
		...
		public function updateNetworkSetting(){
			extract($_POST);
			$check=$this->model->updateNetworkSetting($network,$general,$vtuStatus,$sharesellStatus,$airtimepin,$datapin,$sme,$gifting,$corporate,$networkid,$vtuId,$sharesellId,$smeId,$giftingId,$corporateId,$sme2,$sme2Id,$coupon,$couponId,$rId,$dId,$dataId,$data);
		...
		public function updateSiteSetting(){
			extract($_POST);
			$check=$this->model->updateSiteSetting($sitename,$siteurl,$apidocumentation,$referalupgradebonus,$referalairtimebonus,$referaldatabonus,$referalwalletbonus,$referalcablebonus,$referalexambonus,$referalmeterbonus,$wallettowalletcharges,$agentupgrade,$vendorupgrade,$accountname,$accountno,$bankname,$electricitycharges,$airtimemin,$airtimemax,$smilediscount,$kycoption,$kycbvncharges,$kycnincharges,$kycShouldEnable,$kycShouldVerify,$registrationEnable);
	...
	//Delete Data Plan
		public function deleteFlashNoti(){
			extract($_POST);
			$check=$this->model->deleteFlashNoti($id);
			if($check == 0){return 0;}
			else{return 1;}
		}
		...
		public function getWalletBalance(){
			...
			$walletSeven = $model->getWalletBalance("seven",$details);
			$walletEight = $model->getWalletBalance("eight",$details);
			$walletNine = $model->getWalletBalance("nine",$details);
			...
			$data["walletSevenBalance"] = $walletSeven["balance"]; $data["walletSevenProvider"] = $walletSeven["provider"];
			$data["walletEightBalance"] = $walletEight["balance"]; $data["walletEightProvider"] = $walletEight["provider"];
			$data["walletNineBalance"] = $walletNine["balance"]; $data["walletNineProvider"] = $walletNine["provider"];
			...
			$walletSevenBalance = (float) str_replace(",","",$walletSeven["balance"]);
			$walletEightBalance = (float) str_replace(",","",$walletEight["balance"]);
			$walletNineBalance = (float) str_replace(",","",$walletNine["balance"]);
			

			$walletTotal =  $walletOneBalance +  $walletTwoBalance + $walletThreeBalance + $walletFourBalance + $walletFiveBalance + $walletSixBalance + $walletSevenBalance + $walletEightBalance + $walletNineBalance;
	...
	public function getFlashNoti(){
			$data=$this->model->getFlashNoti();
			return $data;
		}

			//Add Notification
		public function addFlashNoti(){
			extract($_POST);
			$check=$this->model->addFlashNoti($subject,$message);
			if($check == 0){return $this->createNotification1("alert-success","Faq Added Successfully");}
			else{return $this->createNotification1("alert-danger","Unable To Add Faq, Please Try Again");}
			
		}	
		
		// Function to get all mappings
public function getDataTypeMappings($returnData = false) {
    $mappings = $this->model->getDataTypeMappings();
    
    if ($returnData) {
        return $mappings; // Return the data directly for use in views
    } else {
...
public function mapDataType() {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $mappedValue = $_POST['mapped_value'] ?? null;

        if (!$dataType || !$mappedValue) {
            echo json_encode(["status" => "error", "message" => "All fields are required"]);
            return;
        }

        $result = $this->model->mapDataType($dataType, $mappedValue);

        if ($result) {
            echo json_encode(["status" => "success", "message" => "Data type mapping updated successfully"]);
        } else {
            echo json_encode(["status" => "error", "message" => "Failed to update mapping"]);
        }
        return;
    }
}
