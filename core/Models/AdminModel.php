<?php
	...
	class AdminModel extends Model{
		...
		public function updateSiteSetting($sitename,$siteurl,$apidocumentation,$referalupgradebonus,$referalairtimebonus,$referaldatabonus,$referalwalletbonus,$referalcablebonus,$referalexambonus,$referalmeterbonus,$wallettowalletcharges,$agentupgrade,$vendorupgrade,$accountname,$accountno,$bankname,$electricity,$airtimemin,$airtimemax,$smilediscount,$kycoption,$kycbvncharges,$kycnincharges,$kycShouldEnable,$kycShouldVerify,$registrationEnable){
			...
			$query->bindParam(':registrationEnable',$registrationEnable,PDO::PARAM_STR);//Added by developer veeplug
			$query->execute();
			return 0;
		}
		...
		public function updateNetworkSetting($network,$general,$vtuStatus,$sharesellStatus,$airtimepin,$datapin,$sme,$gifting,$corporate,$networkid,$vtuId,$sharesellId,$smeId,$giftingId,$corporateId,$sme2,$sme2Id,$coupon,$couponId,$rId,$dId,$dataId,$data){
			$dbh=$this->connect();
			$id = (float) $network;
			
			$sql="UPDATE networkid SET networkStatus=:g, vtuStatus=:vs, sharesellStatus=:sss, airtimepinStatus=:ap, datapinStatus=:dp, smeStatus=:s, giftingStatus=:gi, sme2Status=:b, corporateStatus=:c, couponStatus=:cds, networkId=:nid, vtuId=:vtuid, sharesellId=:sharesellid,smeId=:smeid,giftingId=:giftid,sme2Id=:sme2id,corporateId=:ccid,rId=:rid,dId=:did,couponId=:cid,dataId=:datai,dataStatus=:data WHERE nId = $id";
			$query = $dbh->prepare($sql); 
			...
			$query->bindParam(':b',$sme2,PDO::PARAM_STR);
			$query->bindParam(':cds',$coupon,PDO::PARAM_STR); 
			$query->bindParam(':cid',$couponId,PDO::PARAM_STR);
			$query->bindParam(':rid',$rId,PDO::PARAM_STR);
			$query->bindParam(':did',$dId,PDO::PARAM_STR);
			$query->bindParam(':datai',$dataId,PDO::PARAM_STR);
		...
		public function getAirtimePinStocks(){
			$dbh=$this->connect();
			$sql = "SELECT * FROM airtimepinstock a, networkid b WHERE a.network=b.rId ORDER BY date DESC";
            $query = $dbh->prepare($sql);
            $query->execute();
            $results=$query->fetchAll(PDO::FETCH_OBJ);
            return $results;
		...
		public function getDataPins(){
			$dbh=$this->connect();
			$sql = "SELECT * FROM datapins a, networkid b WHERE a.datanetwork=b.dId";
            $query = $dbh->prepare($sql);
            $query->execute();
            $results=$query->fetchAll(PDO::FETCH_OBJ);
            return $results;
		...
		///Delete A Data Plan
		public function deleteFlashNoti($id){
			$dbh=$this->connect();
			$sql = "DELETE FROM flash_noti WHERE msgId=$id";
            $query = $dbh->prepare($sql);
            $query->execute();
            return 0;
		}
		...
		public function addDataPin($network,$dataname,$datatype,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$loadpin,$checkbal){
			...
			//If Not Exist, Create New User
			$sql="INSERT INTO  datapins (datanetwork,name,type,planid,day,price,userprice,agentprice,vendorprice,loadpin,checkbal) 
		...
		public function updateDataPin($pin,$network,$dataname,$datatype,$planid,$duration,$price,$userprice,$agentprice,$vendorprice,$loadpin,$checkbal){
			...
			$sql="UPDATE datapins SET datanetwork=:n,name=:d,type=:dt,planid=:p,day=:du,price=:pr,userprice=:up,agentprice=:ap,vendorprice=:vp,loadpin=:lp,checkbal=:cb WHERE dpId=:id";
            $query = $dbh->prepare($sql);
	...
	public function getFlashNoti(){
			$dbh=self::connect();
			$sql = "SELECT * FROM flash_noti";
            $query = $dbh->prepare($sql);
            $query->execute();
            $results=$query->fetchAll(PDO::FETCH_OBJ);
            return $results;
				...
				public function addFlashNoti($subject,$message){
			$dbh=self::connect();
			$sql="INSERT INTO flash_noti SET subject=:s,message=:m";
			$query = $dbh->prepare($sql);
			$query->bindParam(':s',$subject,PDO::PARAM_STR);
			$query->bindParam(':m',$message,PDO::PARAM_STR);
			$query->execute();
...
public function mapDataType($dataType, $mappedValue) {
    $allowedTypes = ["SME", "Gifting", "Sme2", "Data Share", "Coupon", "Corporate"];
    
    if (!in_array($dataType, $allowedTypes)) {
        return false; // Prevent modifying non-allowed data types
    }
    
    try {
        $dbh = $this->connect(); // Get database connection
        $query = "UPDATE data_type_mapping SET mapped_value = ? WHERE data_type = ?";
        $stmt = $dbh->prepare($query);
        return $stmt->execute([$mappedValue, $dataType]);
    } catch (PDOException $e) {
        error_log("Database error in mapDataType: " . $e->getMessage());
        return false;
    }
}
...
public function getDataTypeMappings() {
    try {
        $dbh = $this->connect(); // Get database connection
        $sql = "SELECT * FROM data_type_mapping";
        $query = $dbh->prepare($sql);
        $query->execute();
        $results = $query->fetchAll(PDO::FETCH_OBJ); // Using FETCH_OBJ to match your style
        return $results;
    } catch (PDOException $e) {
        error_log("Database error in getDataTypeMappings: " . $e->getMessage());
        return [];
    }
}
