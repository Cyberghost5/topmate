<?php
	...
	//Verify Networkfe Id
        ...
        // Record Bellbank Transaction
public function recordUnitedapiTransaction($userid, $servicename, $servicedesc, $ref, $amountopay, $oldbalance, $newbalance, $status) {
    $dbh = $this->connect();
    $userid = (float) $userid;
    $status = (float) $status;
    $date = date("Y-m-d H:i:s");
    ...
    logMessage("Recording uapi Transaction: User ID: $userid, Service Name: $servicename, Amount to Pay: $amountopay, Old Balance: $oldbalance, New Balance: $newbalance, Ref: $ref Status: $status");
    ...
    if ($queryC->rowCount() > 0) {
        logMessage("Transaction already exists with reference: $ref");
        return 1;
    }
    ...
    logMessage("Wallet updated for User ID: $userid. New Balance: $newbalance");
    ...
    logMessage("Transaction recorded for User ID: $userid. Transaction Reference: $ref, Service: $servicename, Status: $status");

    $lastInsertId = $dbh->lastInsertId();
    if ($lastInsertId) {
        logMessage("Transaction recorded successfully with ID: $lastInsertId");
        return 0;
    } else {
        logMessage("Failed to record transaction for User ID: $userid with reference: $ref");
        return 1;
    }
}

public function verifyUnitedapiRef($account_number, $token, $payload) {
    try {
        $dbh = $this->connect();
        
        // Get Api Key and Configuration Details
        $sql = "SELECT * FROM apiconfigs";
        $query = $dbh->prepare($sql);
        $query->execute(); 
        $result = $query->fetchAll(PDO::FETCH_OBJ);
    
        $bellbankSecret = $this->getConfigValue($result, "bellbankSecret");
        $bellbankCharges = (float) $this->getConfigValue($result, "bellbankCharges");
        $bellbankChargesType = $this->getConfigValue($result, "bellbankChargesType");

        // Directly proceed without computing the hash or verifying the signature
        // Just log the received payload for reference
        logMessage("Received Payload: " . json_encode($payload));
        
        // Query the user based on account number (sBillstackBank)
        $sqlA = "SELECT * FROM subscribers WHERE sBellBank=:account_number";
        $queryA = $dbh->prepare($sqlA);
        $queryA->bindParam(':account_number', $account_number, PDO::PARAM_STR);
        $queryA->execute();
        $resultA = $queryA->fetch(PDO::FETCH_OBJ);
        
        // Log the account number check result
        if ($queryA->rowCount() > 0) {
            ...
            $response["charges"] = $bellbankCharges;
            $response["chargestype"] = $bellbankChargesType;
            
            logMessage("User found: " . $resultA->sEmail);
            
            return (object) $response;
        } else {
            // No user found, return fail
            $response = array();
        ...
        public function deleteTransactionDueToInsufficientBalance($ref){
         ...
         public function verifyBillstackNotification($key,$email){
            ...
            $asfiyKey = $this->getConfigValue($result,"billstackSecretKey");
            
            $asfiyCharges = (float) $this->getConfigValue($result,"billstackCharges");
            $asfiyChargesType = $this->getConfigValue($result,"billstackChargesType");
...
if ($sessionId == "$sessionId") {
  ...
  CURLOPT_URL => "https://strowallet.com/api/verifysessionid/?public_key=$strowalletPublicKey&amount=$amount_paid&sessionID=$sessionId",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 30,
