<?php
	...
	class SubscriberModel extends Model{
		...
		public function updateProfileKey($id,$oldKey,$newKey){
			...
			//Get Billstack Accounts for a Subscriber
		public function getBillstackAccounts($userid){
			
			$dbh=$this->connect();
			$userid=(float) $userid;
		
			$c="SELECT * FROM  billstack_accounts WHERE userid=:p";
	    	$queryC = $dbh->prepare($c);
	    	$queryC->bindParam(':p',$userid,PDO::PARAM_STR);
	     	$queryC->execute();
	      	$result=$queryC->fetch(PDO::FETCH_OBJ);
	      	...
	      	if($queryC->rowCount() > 0){
	           return $result;
	      	    ...
	      	    return 1;//It means user doesnot have one yet
		...
		public function generateBellbankAccount($id){
			...
			if(empty($result->sBellBank)){
				...
				$d=$this->getApiConfiguration();
				$bellbankkStatus = $this->getConfigValue($d,"bellbankStatus"); 
				

				//If Payvessel Status Is Active, Create Virtual Account For User
				if($bellbankkStatus == "On"){
					$obj->generateBellbankAccount($id,$result->sFname,$result->sLname,$result->sPhone,$result->sEmail);
				}
		...
		public function getDataPins(){
			$dbh=$this->connect();
			$sql = "SELECT * FROM datapins a, networkid b WHERE a.datanetwork = b.dId ORDER BY a.dpId ASC";
            $query = $dbh->prepare($sql);
            $query->execute();
            $results=$query->fetchAll(PDO::FETCH_OBJ);
            return $results;
...
function getSpent($period, $userId) {
    ...
    return $totalAmountSpent;
		...
		public function sendManualFundingRequest($payaccount,$paymethod,$payamount,$userId){
			...
			//Get Strowallet Accounts for a Subscriber
		public function getStrowalletAccounts($userid){
			
			$dbh=$this->connect();
			$userid=(float) $userid;
		
			$c="SELECT * FROM  strowallet_accounts WHERE userid=:p";
	    	$queryC = $dbh->prepare($c);
	    	$queryC->bindParam(':p',$userid,PDO::PARAM_STR);
	     	$queryC->execute();
	      	$result=$queryC->fetch(PDO::FETCH_OBJ);
...
//----------------------------------------------------------------------------------------------------------------
		// BANKLY ACCOUNT MANAGEMENT
		//----------------------------------------------------------------------------------------------------------------
		
		//Generate Bankkly Account
		
		public function generateStrowalletAccount($id,$account_type){
		    
		
			$id = (float) $id;
			$dbh=$this->connect();
	    	$sql="SELECT * FROM strowallet_accounts WHERE userid = $id";
			$queryC = $dbh->prepare($sql);
	    	$queryC->execute();
	      	$result=$queryC->fetch(PDO::FETCH_OBJ);
	      		 file_put_contents('logstro_file.txt',json_encode($result));
	      		//Get User Details
		      	$d=$this->getApiConfiguration();
	      	...
	      	if($queryC->rowCount() > 0){
				...
				case "bankly":
				$bank=$result->bankly;
				$bankStatus = $this->getConfigValue($d,"banklyStatus");
				break;
				case "safehaven":
				$bank=$result->safehaven;
				$bankStatus = $this->getConfigValue($d,"safehavenStatus");
				break;
				case "paga":
				$bank=$result->paga;
				$bankStatus = $this->getConfigValue($d,"pagaStatus");
				break;
			...
			if(empty($bank) || $bank== null){
				...
				$user=$this->getProfileInfo($id);
				
				if($bankStatus == "On"){
			$generateBank=$obj->generateStrowalletAccount($id,$user->sFname,$user->sLname,$user->sPhone,$user->sEmail,$account_type);
			return $generateBank;
				}
		...
		public function generateKudaAccount($id){
            ...
            //----------------------------------------------------------------------------------------------------------------
		// PAGA ACCOUNT MANAGEMENT By Daltech Digital Resources 07035890194
		//----------------------------------------------------------------------------------------------------------------
	...
	//------------------------------------------------------------------
//Update Profile Password
		public function saveBillstackVirtualAccount($userId,$accountNumber,$ref,$bank){
			...
			//Check if already recorded in database if no insert else update
			
			$dbh=$this->connect();
	    	$sql="SELECT * FROM billstack_accounts WHERE userid =$userid";
			$queryC = $dbh->prepare($sql);
	    	$queryC->execute();
	      	$result=$queryC->fetch(PDO::FETCH_OBJ);
	      	if($queryC->rowCount() > 0){
	          ...
	          // die("$userId,$accountNumber,$ref,$bank lets update");
	            $c="UPDATE billstack_accounts SET active=1,$row,account_reference=:ref WHERE userid=$userid";
				$queryC = $dbh->prepare($c);
			  ...
			  return 0;
	      	    ...
	      	    //Insert
	      	  //  die("lets insert here ooooo $row $userId,$accountNumber,$ref,$bank");
	      	    $active=1;
	      	    $sql="INSERT INTO billstack_accounts SET userid=:user,active =:active,$row,account_reference=:ref";
					$queryC = $dbh->prepare($sql);
					$queryC->bindParam(':user',$userid,PDO::PARAM_INT);
					$queryC->bindParam(':active',$active,PDO::PARAM_STR);
					$queryC->bindParam(':AS',$accountNumber,PDO::PARAM_STR);
					$queryC->bindParam(':ref',$ref,PDO::PARAM_STR);
					$queryC->execute();
					return 0;
   ...
   public function deleteBeneficiary($id, $userid) { 
    ...
    return ($affectedRows > 0);
		...
		public function getFlashNotification(){
			$dbh=$this->connect();
			$sql = "SELECT * FROM flash_noti ORDER BY msgId DESC LIMIT 1";
            $query = $dbh->prepare($sql);
			$query->execute();
            $results=$query->fetch(PDO::FETCH_OBJ);
            return $results;
